...
class StatusBar extends View
  ...
  @content: ->
    @div class: 'faketerm2 status-bar', tabindex: -1, =>
  ...
  initialize: (@statusBarProvider) ->
    ...
    @subscriptions.add atom.commands.add 'atom-workspace',
      'faketerm2:new': => @newTerminalView()
      'faketerm2:toggle': => @toggle()
      'faketerm2:next': =>
      ...
      'faketerm2:close': => @destroyActiveTerm()
      'faketerm2:close-all': => @closeAll()
      'faketerm2:rename': => @runInActiveView (i) -> i.rename()
      'faketerm2:insert-selected-text': => @runInActiveView (i) -> i.insertSelection('$S')
      'faketerm2:insert-text': => @runInActiveView (i) -> i.inputDialog()
      'faketerm2:insert-custom-text-1': => @runInActiveView (i) -> i.insertSelection(atom.config.get('faketerm2.customTexts.customText1'))
      'faketerm2:insert-custom-text-2': => @runInActiveView (i) -> i.insertSelection(atom.config.get('faketerm2.customTexts.customText2'))
      'faketerm2:insert-custom-text-3': => @runInActiveView (i) -> i.insertSelection(atom.config.get('faketerm2.customTexts.customText3'))
      'faketerm2:insert-custom-text-4': => @runInActiveView (i) -> i.insertSelection(atom.config.get('faketerm2.customTexts.customText4'))
      'faketerm2:insert-custom-text-5': => @runInActiveView (i) -> i.insertSelection(atom.config.get('faketerm2.customTexts.customText5'))
      'faketerm2:insert-custom-text-6': => @runInActiveView (i) -> i.insertSelection(atom.config.get('faketerm2.customTexts.customText6'))
      'faketerm2:insert-custom-text-7': => @runInActiveView (i) -> i.insertSelection(atom.config.get('faketerm2.customTexts.customText7'))
      'faketerm2:insert-custom-text-8': => @runInActiveView (i) -> i.insertSelection(atom.config.get('faketerm2.customTexts.customText8'))
      'faketerm2:fullscreen': => @activeTerminal.maximize()

    @subscriptions.add atom.commands.add '.xterm',
      'faketerm2:paste': => @runInActiveView (i) -> i.paste()
      'faketerm2:copy': => @runInActiveView (i) -> i.copy()
    ...
    @subscriptions.add atom.workspace.onDidChangeActivePaneItem (item) =>
      ...
      if item.constructor.name is "PlatformIOTerminalView"
        ...
        mapping = atom.config.get('faketerm2.core.mapTerminalsTo')
        ...
        if prevTerminal != nextTerminal
          if not nextTerminal?
            if item.getTitle() isnt 'untitled'
              if atom.config.get('faketerm2.core.mapTerminalsToAutoOpen')
  ...
  registerContextMenu: ->
    @subscriptions.add atom.commands.add '.faketerm2.status-bar',
      'faketerm2:status-red': @setStatusColor
      'faketerm2:status-orange': @setStatusColor
      'faketerm2:status-yellow': @setStatusColor
      'faketerm2:status-green': @setStatusColor
      'faketerm2:status-blue': @setStatusColor
      'faketerm2:status-purple': @setStatusColor
      'faketerm2:status-pink': @setStatusColor
      'faketerm2:status-cyan': @setStatusColor
      'faketerm2:status-magenta': @setStatusColor
      'faketerm2:status-default': @clearStatusColor
      'faketerm2:context-close': (event) ->
        $(event.target).closest('.pio-terminal-status-icon')[0].terminalView.destroy()
      'faketerm2:context-hide': (event) ->
        statusIcon = $(event.target).closest('.pio-terminal-status-icon')[0]
        statusIcon.terminalView.hide() if statusIcon.isActive()
      'faketerm2:context-rename': (event) ->
  ...
  registerPaneSubscription: ->
    @subscriptions.add @paneSubscription = atom.workspace.observePanes (pane) =>
      ...
      tabBar.on 'dragstart', (event) ->
        ...
        event.originalEvent.dataTransfer.setData 'faketerm2-tab', 'true'
  ...
  createTerminalView: (autoRun) ->
    ...
    switch atom.config.get('faketerm2.core.workingDirectory')
    ...
    shell = atom.config.get 'faketerm2.core.shell'
    shellArguments = atom.config.get 'faketerm2.core.shellArguments'
  ...
  setStatusColor: (event) ->
    ...
    color = atom.config.get("faketerm2.iconColors.#{color}").toRGBAString()
  ...
  onDragStart: (event) =>
    event.originalEvent.dataTransfer.setData 'faketerm2-panel', 'true'
  ...
  onDragOver: (event) =>
    ...
    unless event.originalEvent.dataTransfer.getData('faketerm2') is 'true'
      return
  ...
  onDrop: (event) =>
    ...
    panelEvent = dataTransfer.getData('faketerm2-panel') is 'true'
    tabEvent = dataTransfer.getData('faketerm2-tab') is 'true'
  ...
  onDropTabBar: (event, pane) =>
    {dataTransfer} = event.originalEvent
    return unless dataTransfer.getData('faketerm2-panel') is 'true'
